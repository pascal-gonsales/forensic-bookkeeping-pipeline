...
The Ralf-CC project lost state between sessions. 1,347 transactions processed, $409K still unclassified, and the next session had no way to pick up where it left off. The forensic-bookkeeping project is better (SESSION_STATE.md exists), but it's manually maintained and drifts from reality.
...
- Rules learned (e.g., "SUPPLIER_ALPHASERVICEOWNER_BM" = "Damen" in Excel)
...
```json
{
  "rules": [
    ...
    },
    {
      "id": "rule_002",
      "pattern": "INTEGRAL XT",
      "maps_to": "Integral XT (Exterminateur)",
      "classification": "BUSINESS",
...
### The Problem
You told Claude that "Integral XT = exterminateur = business" in one session. Next session, Claude asks again. This wastes your time and erodes trust.
...
```jsonl
{"id":"d001","timestamp":"2026-03-31T09:15:00","question":"What is Integral XT?","answer":"Exterminateur — business expense, supplier_service","context":"CC classification session, Owner_A confirmed verbally","category":"classification_rule","reversible":true}
...
- Integral XT = exterminateur = BUSINESS (d001, 2026-03-31)
...
| "Integral XT = exterminateur" | Session State (decisions.jsonl) + Skill (once confirmed) | Starts as decision, graduates to rule |
...
```
Session observation → "SUPPLIER_ALPHASERVICEOWNER_BM matches Damen in Excel"
...
3. **Re-asking confirmed decisions**: If Owner_A said "Integral XT = exterminateur" once, that's a permanent rule. Load decisions.jsonl and check before asking anything.
